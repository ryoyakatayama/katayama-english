...
function render() {
  ...
  root.innerHTML = `<header class="app-header"><a href="#" data-action="home" class="brand" ${screen === 'quiz' ? 'aria-disabled="true"' : ''}><img src="./icons/icon-192.png?v=nuasa-mask-20260912" width="42" height="42" alt=""><span>片山英単語<small>NuASA · 2,000 WORDS</small></span></a>${screen !== 'quiz' ? button('settings', `<span class="avatar">${esc(data.profile.name.slice(0, 1))}</span><span class="profile-name">${esc(data.profile.name)}</span>`, 'profile-button', 'aria-label="プロフィールとデータ"') : ''}</header>
